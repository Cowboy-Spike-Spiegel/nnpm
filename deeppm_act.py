...
def get_model(input_length=10, n_filters=3, vocab_size=10, n_classes=9, embedding_size=5, n_modules=5, model_type='ACT', learning_rate=0.002):
    ...
    inputs = []
    for i in range(2):
        inputs.append(kr.layers.Input(shape=(input_length,)))

    inputs_ = []
    for i in range(2):
        if (i==0):
            a = kr.layers.Embedding(vocab_size, embedding_size, input_length=input_length)(inputs[0])
            inputs_.append(kr.layers.Embedding(vocab_size, embedding_size, input_length=input_length)(inputs[i]))
        else:
            inputs_.append(kr.layers.Reshape((input_length, 1))(inputs[i]))

    filters_inputs = kr.layers.Concatenate(axis=2)(inputs_)

    for m in range(n_modules):
        filters = []
        for i in range(n_filters):
            filters.append(kr.layers.Conv1D(filters=32, strides=1, kernel_size=1+i, activation='relu', padding='same')(filters_inputs))
        filters.append(kr.layers.MaxPooling1D(pool_size=3, strides=1, padding='same')(filters_inputs))
        filters_inputs = kr.layers.Concatenate(axis=2)(filters)
        #filters_inputs = kr.layers.Dropout(0.1)(filters_inputs)

    #pool = kr.layers.GlobalAveragePooling1D()(filters_inputs)
    pool = kr.layers.GlobalMaxPooling1D()(filters_inputs)
    #pool = kr.layers.Flatten()(filters_inputs)

    #pool = kr.layers.Dense(64, activation='relu')(pool)


    optimizer = kr.optimizers.Adam(lr=learning_rate)

    if (model_type == 'BOTH'):
        out_a = kr.layers.Dense(n_classes, activation='softmax', name='output_a')(pool)
        out_t = kr.layers.Dense(1, activation='linear', name='output_t')(pool)
        model = kr.models.Model(inputs=inputs, outputs=[out_a, out_t])
        model.compile(optimizer=optimizer, loss={'output_a':'categorical_crossentropy', 'output_t':'mae'})
    else:
        if (model_type=='ACT'):
            out = kr.layers.Dense(n_classes, activation='softmax')(pool)
            model = kr.models.Model(inputs=inputs, outputs=out)
            model.compile(optimizer=optimizer, loss='mse',metrics=['acc'])
        elif (model_type=='TIME'):
            out = kr.layers.Dense(1, activation='linear')(pool)
            model = kr.models.Model(inputs=inputs, outputs=out)
...
def fit_and_score(params):
    ...
    early_stopping = kr.callbacks.EarlyStopping(monitor='val_loss', patience=20)
    """
...
X_t = X_t / np.max(X_t)
# categorical output
y_a = kr.utils.to_categorical(y_a)
...
for f in range(3):
    ...
    trials = Trials()
    rng = np.random.default_rng(seed=seed+f)
    print(rng)
    best = fmin(fit_and_score, space, algo=tpe.suggest, max_evals=n_iter, trials=trials, rstate=rng)
